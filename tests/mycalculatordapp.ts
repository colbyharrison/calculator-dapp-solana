...
describe('mycalculatordapp', () => {
  ...
  it('Division by zero', async function () {
    const calculator = _calculator;
    try {

      await program.rpc.divide(new anchor.BN(2), new anchor.BN(0), {
        accounts: {
          calculator: calculator.publicKey,
        },
      });
      ...
      const errMsg = "You tried to divide by zero. C'mon Now!";
      assert.equal(error.toString(), errMsg);
    }
  });
});